...
const diractionMap: any = { left: 'left', right: 'left', top: 'top', bottom: 'top' };
const multyMap: any = { left: 1, right: -1, top: 1, bottom: -1 };
...
export class DialogAnimateService {
  ...
  open<T, D = any, R = any>(
    ...
    if (config.animation) {
      const incomingOptions: AnimationOption = { keyframeAnimationOptions: { duration: 400, easing: 'ease-in' } };
      ...
      const animate = (keyframes: any, options: any) => wrapper.animate(keyframes, options);
