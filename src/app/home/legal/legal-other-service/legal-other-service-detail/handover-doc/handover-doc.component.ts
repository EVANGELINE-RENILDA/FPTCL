...
@Component({
  selector: 'app-handover-doc',
  standalone: true,
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
  imports: [AccordionModule, ViewLabel, InputControlComponent, FormsModule, IconButtonComponent, Row, FileSizePipe, AppDatePipe, DropZoneComponent, AppTableComponent],
  templateUrl: './handover-doc.component.html',
  styleUrl: './handover-doc.component.scss'
})
export class HandoverDocComponent {
  ...
  async init() {
    ...
    this.application.lstentLegalApplicationNotes.forEach((element: any) => {
      if (element.applicationStageValue === 'HDODC' && element.identificationNo === this.type.legalApplicationIdenityNo) {
  ...
  uploadClientDocumentAttachment(event: any) {
    ...
    this.legalDocument.identificationNo = this.application.identifierCount;
