...
@Component({
  ...
  imports: [AccordionModule, ViewLabel, InputControlComponent, FormsModule, IconButtonComponent, Row, FileSizePipe, AppDatePipe, DropZoneComponent, AppTableComponent, LegalServicePipe, TabSubComponent, TabSubGroupComponent, CellDefDirective, CellHeaderDefDirective]
})
export class DocumentCollectionComponent {
  ...
  async init() {
    ...
    await this.getAssignedOfficerList();
    this.loading = true
    this.application.lstentLegalApplicationNotes.forEach((element: any) => {
      if (element.applicationStageValue === 'CLDOC' && element.identificationNo === this.type.legalApplicationIdenityNo) {
  ...
  cancelApplicationStage() {
    if (this.xForm.valid) {
      ...
      this.apiService
        .saveLegalApplicationNotes(this.application)
        .subscribe((success) => {
          this.application = success;
          this.cancelNotesDescription = success.ientLegalApplicationNotes.notes;
          this.applicationChange.emit(this.application)
          this.dialog.closeAll();
          this.data.successMessage(success.msg.infoMessage.msgDescription);
        });
