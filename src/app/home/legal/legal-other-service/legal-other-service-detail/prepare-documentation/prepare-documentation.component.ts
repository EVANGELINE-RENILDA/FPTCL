...
@Component({
  selector: 'app-prepare-documentation',
  standalone: true,
  imports: [AccordionModule, ViewLabel, InputControlComponent, FormsModule, IconButtonComponent, Row, FileSizePipe, AppDatePipe, AppTableComponent, DropZoneComponent, CellDefDirective, CellHeaderDefDirective, TabSubComponent, TabSubGroupComponent, LegalServicePipe],
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
  templateUrl: './prepare-documentation.component.html',
  styleUrl: './prepare-documentation.component.scss'
})
export class PrepareDocumentationComponent {
  ...
  notesMessage = ''
  async init() {
    await this.documentTypeDDLdata();
    this.getAssignedOfficerList();
    this.loading = true
    this.application.lstentLegalApplicationNotes.forEach((element: any) => {
      if (element.applicationStageValue === 'PREDC' && element.identificationNo === this.type.legalApplicationIdenityNo) {
