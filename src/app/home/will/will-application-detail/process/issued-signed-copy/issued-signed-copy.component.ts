...
@Component({
  selector: 'app-issued-signed-copy',
  standalone: true,
  imports: [
    FormsModule,
  ...
  templateUrl: './issued-signed-copy.component.html',
  styleUrl: './issued-signed-copy.component.scss',
})
export class IssuedSignedCopyComponent {
  ...
  async init() {
    ...
    if (this.application.lstentWillApplicationNotes.length !== 0) {
      this.application.lstentWillApplicationNotes.forEach((element: any) => {
        if (
          element.applicationStatusValue === 'ISIGT' &&
  ...
  downloadImageFile() {
    ...
    if (this.application.applicationStageValue === 'RDRTS') {
    }
  }
