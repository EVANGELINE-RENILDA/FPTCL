...
@Component({
  selector: 'app-collect-beneficiary-doc',
  standalone: true,
  imports: [
    FormsModule,
  ...
  templateUrl: './collect-beneficiary-doc.component.html',
  styleUrl: './collect-beneficiary-doc.component.scss',
})
export class CollectBeneficiaryDocComponent {
  ...
  init() {
    ...
    if (this.application.lstentWillApplicationNotes.length !== 0) {
      this.application.lstentWillApplicationNotes.forEach((element: any) => {
        if (
          element.applicationStageValue === 'BNDOC' &&
