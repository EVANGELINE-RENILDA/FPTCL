...
@Component({
  selector: 'app-collect-will-register',
  standalone: true,
  imports: [
    AccordionModule,
    FormsModule,
  ...
  templateUrl: './collect-will-register.component.html',
  styleUrl: './collect-will-register.component.scss',
})
export class CollectWillRegisterComponent {
  ...
  downloadImageFile() {
    ...
    this.application.lstentWillApplicationNotes.forEach((element: any) => {
      if (element.applicationStageValue === 'CLREG' && element.identityNo === this.type.identityNo) {
