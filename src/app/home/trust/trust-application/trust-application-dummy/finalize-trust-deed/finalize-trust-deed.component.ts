...
@Component({
  selector: 'app-finalize-trust-deed',
  standalone: true,
  imports: [AccordionModule, Row, AppDatePipe, FormsModule, DropZoneComponent, AppTableComponent, CellDefDirective, CellHeaderDefDirective, ViewLabel, IconButtonComponent, InputControlComponent, CheckboxComponent, CommonModule, TrustApplicationPipePipe, TabSubGroupComponent, TabSubComponent, FileSizePipe],
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
  templateUrl: './finalize-trust-deed.component.html',
  styleUrl: './finalize-trust-deed.component.scss'
})
export class FinalizeTrustDeedComponent {
  ...
  async init() {
    ...
    this.application.lstentTrustApplicationNotes.forEach((element: any) => {
      if (element.applicationStageValue === 'FTRDE' && element.identityNo === this.type.trustApplicationIdenityNo) {
  ...
  saveNotesWithoutParent() {
    ...
    this.application.ientTrustApplicationNotes.trustApplicationId = this.application.trustApplicationId;
