...
@Component({
  selector: 'app-approval-by-ml',
  standalone: true,
  imports: [FormsModule, InputControlComponent,ViewLabel, AppDatePipe,Row, AccordionModule, IconButtonComponent],
  schemas:[CUSTOM_ELEMENTS_SCHEMA,],
  templateUrl: './approval-by-ml.component.html',
  styleUrl: './approval-by-ml.component.scss'
})
export class ApprovalByMlComponent {
  ...
  async init(){
    ...
    if (this.application.lstentTrustApplicationNotes.length !== 0) {
      this.application.lstentTrustApplicationNotes.forEach((element: any) => {
        if (element.applicationStageValue === 'APPML' && element.identityNo === this.type.trustApplicationIdenityNo) {
