...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, InputControlComponent, Row, ViewLabel, AccordionModule, FileSizePipe, AppDatePipe, IconButtonComponent, AppTableComponent, CellDefDirective, CellHeaderDefDirective, DropZoneComponent, TrustApplicationPipePipe, TabSubGroupComponent, TabSubComponent],
...
export class InitialDocumentComponent {
  ...
  async init() {
    ...
    this.application.lstentTrustApplicationNotes.forEach((element: any) => {
      if (element.applicationStageValue === 'RCIDC' && element.identityNo === this.type.trustApplicationIdenityNo) {
