...
@Component({
  selector: 'app-review-open-file',
  standalone: true,
  imports: [FormsModule, InputControlComponent,ViewLabel, AppDatePipe,Row, AccordionModule, IconButtonComponent],
  schemas:[CUSTOM_ELEMENTS_SCHEMA,],
  templateUrl: './review-open-file.component.html',
  styleUrl: './review-open-file.component.scss'
})
export class ReviewOpenFileComponent implements OnInit {
  ...
  async init(){
    ...
    if (this.application.lstentTrustApplicationNotes.length !== 0) {
      this.application.lstentTrustApplicationNotes.forEach((element: any) => {
        if (element.applicationStageValue === 'ROPNF' && element.identityNo === this.type.trustApplicationIdenityNo) {
