...
@Component({
  selector: 'app-filling-doc',
  standalone: true,
  imports: [FormsModule, InputControlComponent,ViewLabel, AppDatePipe,Row, AccordionModule, IconButtonComponent],
  schemas:[CUSTOM_ELEMENTS_SCHEMA,],
  templateUrl: './filling-doc.component.html',
  styleUrl: './filling-doc.component.scss'
})
export class FillingDocComponent {
  ...
  async init(){
    ...
    if (this.application.lstentTrustApplicationNotes.length !== 0) {
      this.application.lstentTrustApplicationNotes.forEach((element: any) => {
        if (element.applicationStageValue === 'FLDOC' && element.identityNo === this.type.trustApplicationIdenityNo) {
