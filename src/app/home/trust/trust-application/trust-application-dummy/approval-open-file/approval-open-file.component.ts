...
@Component({
  selector: 'app-approval-open-file',
  standalone: true,
  imports: [FormsModule, InputControlComponent,ViewLabel, AppDatePipe,Row, AccordionModule,IconButtonComponent],
  schemas:[CUSTOM_ELEMENTS_SCHEMA,],
  templateUrl: './approval-open-file.component.html',
  styleUrl: './approval-open-file.component.scss'
})
export class ApprovalOpenFileComponent {
  ...
  async init(){
    ...
    if (this.application.lstentTrustApplicationNotes.length !== 0) {
      this.application.lstentTrustApplicationNotes.forEach((element: any) => {
        if (element.applicationStageValue === 'AOPNF' && element.identityNo === this.type.trustApplicationIdenityNo) {
