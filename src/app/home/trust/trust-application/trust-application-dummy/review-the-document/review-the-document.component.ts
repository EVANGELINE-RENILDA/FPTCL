...
@Component({
  selector: 'app-review-the-document',
  standalone: true,
  imports: [FormsModule, InputControlComponent,ViewLabel, AppDatePipe,Row, AccordionModule,IconButtonComponent],
  schemas:[CUSTOM_ELEMENTS_SCHEMA,],
  templateUrl: './review-the-document.component.html',
  styleUrl: './review-the-document.component.scss'
})
export class ReviewTheDocumentComponent {
  ...
  async init(){
    ...
    if (this.application.lstentTrustApplicationNotes.length !== 0) {
      this.application.lstentTrustApplicationNotes.forEach((element: any) => {
        if (element.applicationStageValue === 'RWDTS' && element.identityNo === this.type.trustApplicationIdenityNo) {
