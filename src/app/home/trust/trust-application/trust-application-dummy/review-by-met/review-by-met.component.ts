...
@Component({
  selector: 'app-review-by-met',
  standalone: true,
  imports: [FormsModule, InputControlComponent,ViewLabel, AppDatePipe,Row, AccordionModule, IconButtonComponent],
  schemas:[CUSTOM_ELEMENTS_SCHEMA,],
  templateUrl: './review-by-met.component.html',
  styleUrl: './review-by-met.component.scss'
})
export class ReviewByMetComponent {
  ...
  async init(){
    ...
    if (this.application.lstentTrustApplicationNotes.length !== 0) {
      this.application.lstentTrustApplicationNotes.forEach((element: any) => {
        if (element.applicationStageValue === 'RWMET' && element.identityNo === this.type.trustApplicationIdenityNo) {
