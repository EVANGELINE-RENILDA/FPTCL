...
@Component({
  selector: 'app-preparation-of-legal-doc',
  standalone: true,
  imports: [
    AccordionModule,
    FormsModule,
    ...
    CellHeaderDefDirective,
    IconButtonComponent,
    TrustApplicationPipePipe,
    TabSubGroupComponent,
    TabSubComponent,
  ],
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
  templateUrl: './preparation-of-legal-doc.component.html',
  styleUrl: './preparation-of-legal-doc.component.scss',
})
export class PreparationOfLegalDocComponent {
  ...
  async init() {
    ...
    this.application.lstentTrustApplicationNotes.forEach((element: any) => {
      if (
        element.applicationStageValue === 'PRLDT' &&
        element.identityNo === this.type.trustApplicationIdenityNo
      ) {
  ...
  documentTypeDDLdata() {
    ...
    this.apiService
      .getListofDocumentsByTrustType(obj)
  ...
  uploadClientDocumentAttachment(event: any) {
    console.log(event[0]);
    this.trustApplicationDocument.trustApplicationRefNo =
      this.application.trustApplRefNo;
    this.trustApplicationDocument.trustApplicationId =
      this.application.trustApplicationId;
    ...
    this.trustApplicationDocument.ientFile.content = event[0].content;
    this.trustApplicationDocument.trustTypeValue =
      this.application.trustTypeValue;
    this.trustApplicationDocument.applicationStageValue =
    ...
    if (this.trustApplicationDocument.documentTypeValue !== '') {
      ...
      this.apiService
        .saveTrustApplicationDocument(this.trustApplicationDocument)
        .subscribe((success: any) => {
          this.application.lstentTrustApplicationDocument =
            success.lstentTrustApplicationDocument;
          this.docNameType = '';
  ...
  completeApplicationStage() {
    ...
    this.apiService
      .completeTrustApplicationStage(this.application)
      .subscribe((success) => {
  ...
  async checkDocumentDelete(docId: any) {
    ...
    if (response) {
      ...
      this.apiService
        .deleteTrustApplicationDocument(obj)
        .subscribe((success: any) => {
          this.application.lstentTrustApplicationDocument = [];
  ...
  saveNotesWithoutParent() {
    this.application.ientTrustApplicationNotes.notes = this.notesMessage;
    this.application.ientTrustApplicationNotes.actionStatusValue =
      this.application.actionStatusValue;
    this.application.ientTrustApplicationNotes.applicationStageValue =
      this.application.applicationStageValue;
    this.application.ientTrustApplicationNotes.statusValue =
      this.application.statusValue;
    this.application.ientTrustApplicationNotes.trustApplicationId =
      this.application.trustApplicationId;
    if (this.clientDocNotes.valid) {
      this.apiService
        .saveTrustApplicationNoteWithOutParent(this.application)
        .subscribe((success) => {
          this.application = success;
          this.data.successMessage(success.msg.infoMessage.msgDescription);
        });
  ...
  clearNoteswithoutParent() {
    this.application.ientTrustApplicationNotes.notes = '';
  ...
  downloadLinkedFile(val: any) {
    ...
    this.apiService
      .downLoadGeneratedDocuments(obj, options)
      .subscribe((success) => {
  ...
  getAssignOfficerDDL() {
    ...
    this.apiService
      .getListOfOfficerTrustApplication(obj)
      .subscribe((success: any) => {
        this.assignedOfficerDDL = success.value;
      });
  }
  reAssignTrust() {
    this.trustApplicationAssignedOfficer.trustApplicationId =
      this.application.trustApplicationId;
    this.application.ientTrustApplicationAssignedOfficer =
      this.trustApplicationAssignedOfficer;
    this.apiService
      .reAssignTrustApplicationAssignedHistory(this.application)
      .subscribe((success: any) => {
        this.application = success;
      });
  }
  selfAssignTrust() {
    ...
    this.application.ientTrustApplicationAssignedOfficer =
      this.trustApplicationAssignedOfficer;
    this.apiService
      .selfAssignTrustApplicationAssignedHistory(this.application)
      .subscribe((success: any) => {
        this.application = success;
      });
  }
  ...
  cancelApplicationStage() {
    ...
    if (this.gForm.valid) {
      this.apiService
        .saveTrustApplicationNotes(this.application)
        .subscribe((success) => {
          this.application = success;
          this.data.successMessage(success.msg.infoMessage.msgDescription);
        });
