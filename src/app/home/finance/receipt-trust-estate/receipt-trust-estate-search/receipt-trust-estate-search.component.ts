...
@Component({
  ...
  templateUrl: './receipt-trust-estate-search.component.html',
  styleUrl: './receipt-trust-estate-search.component.scss',
})
export class ReceiptTrustEstateSearchComponent {
  ...
  async init() {
    ...
    if (this.type === 'new') {
      ...
      await this.initialData.getDDL(
        'getInitialDataForTranscationPostingforReceiptChangeInDetail'
      );
    }
  ...
  createPostingSearch() {
    ...
    this.apiService
      .createNewTransactionPostingSearch(obj)
      .subscribe((success) => {
  ...
  searchPosting(type?: any) {
    if (this.type === 'new') {
      this.searchParams.voucherTypeValue = 'RECPT';
    } else if (this.type === 'change') {
      this.searchParams.voucherTypeValue = 'RCHDT';
    } else if (this.type === 'reversal') {
      this.searchParams.voucherTypeValue = 'RERCP';
    }

    this.apiService
      .searchTransactionPosting(this.searchParams)
      .subscribe((success) => {
  ...
  getPostingMatrixByEventandSubEventId(val: any) {
    ...
    this.apiService
      .getPostingMatrixByEventandSubEventId(obj)
      .subscribe((success) => {
  ...
  getPaymentTrustAndEstateVouchersCommonDDL() {
    if (this.type === 'new') {
      this.apiService
        .getReceiptTrustAndEstateVouchersCommonDDL()
        .subscribe((success) => {
          this.documentddl = success.data[0].value;
        });
    } else if (this.type === 'change') {
      this.apiService
        .getReceiptChangeInDetailTrustAndEstateVouchersCommonDDL()
        .subscribe((success) => {
          this.documentddl = success.data[0].value;
        });
    } else if (this.type === 'reversal') {
      this.apiService
        .getReceiptReversalTrustAndEstateVouchersCommonDDL()
        .subscribe((success) => {
          this.documentddl = success.data[0].value;
        });
    }
  ...
  async createNewTransaction() {
    if (this.lform.valid) {
      const obj = {
        data: this.searchParams.voucherConfigId,
        data1: this.searchParams.changeInDocumentId,
        type: this.type,
      };
      let urlJson = await this.urlService.encode(obj);
      this.router.navigateByUrl(
        '/home/finance/receipt-trust-and-estate/detail/' + urlJson
      );
    } else {
