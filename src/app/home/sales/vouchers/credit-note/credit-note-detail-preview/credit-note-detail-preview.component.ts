...
import { entActCreditNote, entGetMailInfo, entSalReceiptNote } from '../../../../../common/api-services/accounts-api/accounts-api.classes';
...
@Component({
  selector: 'app-credit-note-detail-preview',
  standalone: true,
  ...
  templateUrl: './credit-note-detail-preview.component.html',
  styleUrl: './credit-note-detail-preview.component.scss'
})
export class CreditNoteDetailPreviewComponent
{
...
errorTrue=false;
  ...
  urlFileName = ''
 pageId = 'PUISC'
  @Input() parentData = new entActCreditNote();
  constructor(
 ...
 viewSalCreditNote() {
  debugger
    if (this.parentData.actCreditNoteId !== 0) {
      const obj = {
        data: this.parentData.actCreditNoteId,
        data1: this.parentData.pientVoucherConfig.pdfOtherComments,
      };
      const options = {
        responseType: 'blob'
      }
