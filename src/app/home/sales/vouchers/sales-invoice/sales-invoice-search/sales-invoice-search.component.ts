...
import { entSalSalesInvoiceSearch, entActVoucherConfig } from '../../../../../common/api-services/accounts-api/accounts-api.classes';
...
@Component({
  ...
  styleUrl: './sales-invoice-search.component.scss'
})
export class SalesInvoiceSearchComponent {

  searchParams = new entSalSalesInvoiceSearch();
  searchParamsRAW = new entSalSalesInvoiceSearch();
  ...
  columns: any = ["docNo", "customerName", "date", "createdDate", "appDate", "docAmount", "reportingAmount", "status", "action"];
  ...
  sortColumnsName: any = ["docNo", "customerName", "date", "createdDate", "appDate", "docAmount", "reportingAmount", "status"];
  ...
  async init() {
    ...
    await this.initialData.getAcccountsDDL('getSalesInvoiceInitialData', this.options);
    this.getNewSalesInvoiceSearch();
    this.totalInprogressandApprovedCounts();
  }

  getNewSalesInvoiceSearch(type?: any) {
    this.accountsService.getNewSalesInvoiceSearch().subscribe((succes) => {
  ...
  searchSalesInvoice() {
    this.searchParams.pageNumber = this.pagination.pageNumber;
    this.searchParams.pageSize = this.pagination.pageSize;
    this.accountsService.searchSalesInvoice(this.searchParams).subscribe((success) => {
      this.pagination.pageNumber = success.pageNumber;
      this.pagination.pageSize = success.pageSize;
      this.pagination.totalCount = success.totalCount;
      this.tableData = success.ilstentSalesInvoiceSearchSet;
    })
  }
    ...
    if (this.lform.valid) {
      if (this.voucherConfigId !== 0) {
        ...
        this.router.navigateByUrl(
          '/home/sales/sales-invoice/detail/' + urlJson
        );
      } else {
  ...
  // async openSaleseInvoice(id: any) {
  //   if (id > 0) {
  //     const obj = {
  //       data: id,
  //       data1: 0,
  ...
  //     this.router.navigateByUrl('/home/sales/sales-invoice/detail/' + encode);
  //   }
  //   else {
  //     this.data.errorInfoMessage('Please select any one record to open');
  //   }
  // }
  async openSalesInvoice(id: any) {
    if (id > 0) {
      const obj = {
        data: id,
        data1: 0,
