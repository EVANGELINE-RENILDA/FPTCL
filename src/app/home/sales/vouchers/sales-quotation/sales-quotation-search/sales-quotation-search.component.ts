...
import { entActVoucherConfig, entPurQuotation, entPurQuotationSearch, entSalSalesQuotationSearch } from '../../../../../common/api-services/accounts-api/accounts-api.classes';
import { AccountsApiService } from '../../../../../common/api-services/accounts-api/accounts-api.service';
import { SearchCustomerComponent } from '../../../../search/search-customer/search-customer.component';

@Component({
  selector: 'app-sales-quotation-search',
  standalone: true,
  templateUrl: './sales-quotation-search.component.html',
  styleUrl: './sales-quotation-search.component.scss',
...
export class SalesQuotationSearchComponent {

  searchParams = new entSalSalesQuotationSearch();
  searchParamsRAW = new entSalSalesQuotationSearch();
  ...
  async init() {
    ...
    await this.initialData.getAcccountsDDL('getSalesQuotationInitialData', this.options);
    this.getNewSalesQuotationSearch();
    this.totalInprogressandApprovedCounts();
  }

  getNewSalesQuotationSearch(type?: any) {
    this.accountsService.getNewSalesQuotationSearch().subscribe((succes) => {
      this.searchParams = succes;
      this.searchParams.regionValue = 'REGI1'
      this.searchParams.pageNumber = this.pagination.pageNumber;
      this.searchParams.pageSize = this.pagination.pageSize;
      this.searchSalesQuotation();
    })
  }

  searchSalesQuotation() {
    this.searchParams.pageNumber = this.pagination.pageNumber;
    this.searchParams.pageSize = this.pagination.pageSize;
    this.accountsService.searchSalesQuotation(this.searchParams).subscribe((success) => {
      this.pagination = success;
      this.tableData = success.ilstentSalSalesQuotationSearchSet;
    })
  }
  doSearchCustomer(event: any) {
    this.customerName = event.customerName;
    this.searchParams.strActCustomerId = event.actCustomerId;
  }
    ...
    if (this.lform.valid) {
      if (this.voucherConfigId !== 0) {
        ...
        this.router.navigateByUrl(
          '/home/sales/sales-quotation/detail/' + urlJson
        );
      } else {
  ...
  async openDetail(id: any) {
    if (id > 0) {
      ...
      this.router.navigateByUrl('/home/sales/sales-quotation/detail/' + encode);
    }
    else {
