...
import { entGetMailInfo, entSalAdvanceReceiptNote } from '../../../../../../common/api-services/accounts-api/accounts-api.classes';
...
@Component({
  selector: 'app-advance-receipt-note-preview',
  standalone: true,
  ...
  templateUrl: './advance-receipt-note-preview.component.html',
  styleUrl: './advance-receipt-note-preview.component.scss'
})
export class AdvanceReceiptNotePreviewComponent {
  @Input('parentId') parentId = '';
  @ViewChild('mailTemplate', { static: false })
  mailTemplate!: TemplateRef<any>;
  @Input() parentData = new entSalAdvanceReceiptNote();
...
errorTrue=false;
  ...
  viewSalAdvanceReceiptNoteReceipt() {
    if (this.parentData.salAdvanceReceiptNoteId !== 0) {
      const obj = {
        data: this.parentData.salAdvanceReceiptNoteId,
