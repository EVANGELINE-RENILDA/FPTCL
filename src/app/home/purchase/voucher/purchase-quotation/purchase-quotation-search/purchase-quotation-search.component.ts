...
import { entActVoucherConfig, entPurQuotation, entPurQuotationSearch } from '../../../../../common/api-services/accounts-api/accounts-api.classes';
...
@Component({
  selector: 'app-purchase-quotation-search',
  standalone: true,
  imports: [Row, InnerScroll, AppTableComponent, IconButtonComponent, AppDatePipe, CellDefDirective, CellHeaderDefDirective, SearchComponent, FormsModule, AppToolTipComponent, InputControlComponent, AccoutsStatusTableComponent, TableStatusSortComponent],
  templateUrl: './purchase-quotation-search.component.html',
  styleUrl: './purchase-quotation-search.component.scss',
  schemas: [CUSTOM_ELEMENTS_SCHEMA]
})
export class PurchaseQuotationSearchComponent {

  searchParams = new entPurQuotationSearch();
  searchParamsRAW = new entPurQuotationSearch();
  ...
  columns: any = ["docNo", "vendorName", "date", "createdDate", "appDate", "docAmount", "repotingAmount", "status", "action"];
  ...
  async init() {
    ...
    this.getNewPurchaseQuotationSearch();
    this.totalInprogressandApprovedCounts();
  }

  getNewPurchaseQuotationSearch(type?: any) {
    this.accountsService.getNewPurchaseQuotationSearch().subscribe((succes) => {
      this.searchParams = succes;
      this.searchParams.regionValue = 'REGI1'
      this.searchParams.pageNumber = this.pagination.pageNumber;
      this.searchParams.pageSize = this.pagination.pageSize;
      this.searchPurchaseQuotation();
    })
  }

  searchPurchaseQuotation() {
    this.searchParams.pageNumber = this.pagination.pageNumber;
    this.searchParams.pageSize = this.pagination.pageSize;
    this.accountsService.searchPurchaseQuotation(this.searchParams).subscribe((success) => {
      this.pagination = success
      this.tableData = success.ilstentPurQuotationSearchResultSet;
  ...
  async openPurchaseQuotation(id: any) {
    if (id > 0) {
      ...
      this.router.navigateByUrl('/home/purchase/purchase-quotation/detail/' + encode);
    }
    else {
