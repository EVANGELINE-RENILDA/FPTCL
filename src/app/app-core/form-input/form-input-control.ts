import { Directive, ElementRef, EventEmitter, Input, Optional, Output, Self, ViewChild, ViewContainerRef } from "@angular/core";
...
export class FormInputControl extends FormAccessorBase {
    ...
    __forceSearch = false;
    @Input() set forceSearch(forceSearch: boolean) { this.__forceSearch = forceSearch || false; }
    get forceSearch() { return this.__forceSearch; }
    ...
    // Input Validation Setting
    __negative = false;
    @Input() set negative(negative: boolean) { this.__negative = negative || false; }
    get negative() { return this.__negative; }
    ...
    set items(items: any) {
        this.__items = items || [];
        this.__items.length > 5
            ? (this.__enableSelectSearch = true)
            : (this.__enableSelectSearch = false);
    }
    get items() {
        return this.__items;
    }
    ...
    setValidate(control: NgControl) {
        ...
        if (this.__valueType === 'int' && this.__decimal === 0 && this.required) {
            let pattern = /^[1-9][0-9]*$/;
    ...
    //footer apply fnc
    doApplyChanges(event: any) {
        this.onApplyChange.next(event);
    }
    // async openOverlay() {
    //     await this.initTimeControl();
